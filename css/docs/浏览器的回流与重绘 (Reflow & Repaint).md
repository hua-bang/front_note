...
2. 游览器将HTML->DOM，CSS->CSSOM,形成Render Tree
3. 有了Render Tree,就知道所有节点的大小和位置，样式，最后将节点绘制在页面上。
...
当页面中元素样式的改变不影响它在文档流的位置（例如：`color`、`background-color`、`visibility`等），浏览器会将新样式赋予给元素并重新绘制它，这个过程称为重绘。
